...
9. Use of AI/ code from other sources:
   - The OCP algorithm is mostly rebuild from the Github repository from the original paper this algorithm
   - is from.
   - There were only some adjustments, i.e. modification in order to use different degree OCPs and a different stopping
   - criterion.
   - The functions in parareal_grundfunktionen.ipynb/ Parareal_algorithm.ipynb are written by me, however AI has been
   - used to spot errors when needed. 
   - AI was also used to generate some very repetitive parts of the code, e.g. the file "propagator.py" in large proportions,
   - or calls of functions to create a plot, where only the name of the input-files/output-files/etc. had to be changed.
